...
AOS.ajax = function (cfg) {
    ...
    if (Ext.isEmpty(cfg.url)) {
    	Ext.log('Ajax请求中的url配置项不能为空。');
        return;
    }
    if(cfg.url.indexOf('do.jhtml') === -1){
    	cfg.url = 'do.jhtml?router=' + cfg.url
    }
    var mycfg = {
        params: params,
        url: cfg.url
...
/**
 * 表单元素赋值方法
 * 用法：(1)、根据表单ID.元素名称的规则设值AOS.setValue('_id_form.name_', 'aos');
 ...
 * @param data :JSON值对象
 */
AOS.setValues = function(formObj, data){
	formObj.form.setValues(data);
}
